...
IS
    ...
    V_TABEL_IMBRICAT TABEL_IMBRICAT := TABEL_IMBRICAT(); -- Inisializare TABEL ÎMBRICAT

BEGIN
    -- 1. Determinarea numarului maxim de productii vizionate de un profil al abonamentului
    FOR R IN (
        ...
        V_VIZIONARI(V_VIZIONARI.COUNT) := R.NUMAR_PRODUCTII; -- Adaugare valoare
    END LOOP;
    ...
    FOR I IN 1 .. V_VIZIONARI.COUNT LOOP
        ...
        END IF;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('Numarul maxim de productii vizionate de un profil al abonamentului ' || P_ABONAMENT_ID || ' este ' || AUXILIAR || '.');

    -- 2. Afisarea rolurilor jucate de actori pentru o productie data
    FOR T IN (
        ...
        V_ACTORI(V_ACTORI.COUNT + 1) := T.ACTOR; -- Adaugare actor
        V_ROLURI(V_ROLURI.COUNT + 1) := T.ROL; -- Adaugare rol
    END LOOP;

    DBMS_OUTPUT.PUT_LINE('Roluri jucate pentru productia ' || P_PRODUCTIE_ID || ':');
    FOR I IN 1 .. V_ACTORI.COUNT LOOP
        DBMS_OUTPUT.PUT_LINE('Actor: ' || V_ACTORI(I) || ', Rol: ' || V_ROLURI(I));
    END LOOP;

    -- 3. Gasirea episoadelor si a directorilor unui serial
    FOR S IN (
        ...
        V_TABEL_IMBRICAT(V_TABEL_IMBRICAT.LAST) := EPISOD_DIRECTOR(S.EPISOD, S.DIRECTOR); -- Adaugare în tabel
    END LOOP;

    DBMS_OUTPUT.PUT_LINE('Episoade si directori pentru serialul ' || P_SERIAL_ID || ':');
...
--------------------------7
--ciclu cursor subcerere in explicit
CREATE OR REPLACE PROCEDURE analiza_vizionari(
    p_abonament_id IN NUMBER -- ID-ul abonamentului pentru care facem analiza
)
IS
    -- Ciclu cursor parametrizat pentru extragerea productiilor vizionate de utilizatorii abonamentului
...
BEGIN
    -- Parcurgem cursorul pentru productii
    ...
    LOOP
        ...
        -- Afisam titlul productiei
        DBMS_OUTPUT.PUT_LINE('Productie vizionata: ' || v_titlu);

        -- Cilcu cursor cu cubcereri simplu pentru extragerea actorilor principali pentru o productie
...
/
BEGIN
    analiza_vizionari(10); -- ID-ul abonamentului pentru care dorim sa analizam vizionarile
END;
/

--------------------8
--Se doreste identificarea numarului total de productii vizionate
--de catre un utilizator care face parte dintr-un anumit
--abonament, împreuna cu detalii despre cea mai vizionata 
--productie a utilizatorului (titlul productiei si numarul de 
--vizionari)
...
BEGIN
    -- Gasim ID-ul profilului asociat cu porecla. Aici se va declansa TOO_MANY_ROWS daca exists mai multe profiluri.
    SELECT 
        profil_id
    INTO 
        v_profil_id
    ...
    -- Calculam numarul total de productii vizionate
    SELECT 
    ...
    -- Calculam titlul celei mai vizionate productii si numarul de vizionari, utilizând ROW_NUMBER
    SELECT 
        titlu, nr_vizionari
    INTO 
    ...
    WHERE rn = 1;  -- Selectam doar productia cu cel mai mare numar de vizionari

    -- Construim rezultatul final
    v_result := 'Utilizatorul cu porecla ' || p_porecla ||  
                ' a vizionat în total ' || v_total_vizionari || ' productii. ' ||CHR(10)||
                'Cea mai vizionata productie este "' || v_titlu_cel_mai_vizionat || 
                '" cu ' || v_nr_vizionari_max || ' vizionari.';
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RETURN 'Nu exista date pentru utilizatorul cu porecla "' || p_porecla || '".';
    WHEN TOO_MANY_ROWS THEN
        RETURN 'Eroare: exista mai multi utilizatori cu porecla "' || p_porecla || 
               '". Verificati unicitatea datelor în baza de date.';
    WHEN OTHERS THEN
...
BEGIN
    DBMS_OUTPUT.PUT_LINE(analiza_utilizator('Spark'));
END;
/
...
---------------------------------------9
--      Daca utilizatorul nu exista, sa se arunce No user found
--      aceasi porecla--too many rows
--      Daca nu exista productii vizionate de utilizator pentru genul specificat, sa se
--arunce o exceptie personalizata NoProductionsForGenreException.
...
BEGIN
    ...
    -- Verifica daca exista productii vizionate de utilizator pentru genul specificat
    SELECT COUNT(*)
...
BEGIN
    IF (TO_CHAR(SYSDATE,'D') IN ('1', '7'))
        OR (MOD(TO_NUMBER(TO_CHAR(SYSDATE, 'MM')), 2)=0)
    THEN
...
--=1 - nu poate fi actualizat
--=0 - poate fi
insert into abonament values(999, 'lunar', to_date('12.12.2024', 'dd.mm.yyyy'));
delete from abonament where abonament_id=999;
...
where abonament_id=40;
--20 mai e expirat pentru test
